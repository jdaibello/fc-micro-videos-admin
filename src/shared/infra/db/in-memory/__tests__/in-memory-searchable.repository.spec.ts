...
describe('InMemorySearchableRepository Unit Tests', () => {
  ...
  describe('applySort method', () => {
    test('should not sort items when sort param is null', () => {
  ...
  describe('search method', () => {
    it('should apply only pagination when other params are null', async () => {
      ...
      expect(result).toStrictEqual(
        new SearchResult({ items: Array(15).fill(entity), total: 16, current_page: 1, per_page: 15 }),
      );
    });

    test('should apply pagination and filter', async () => {
      const items = [
        new StubEntity({ name: 'test', price: 5 }),
        new StubEntity({ name: 'a', price: 5 }),
        new StubEntity({ name: 'TEST', price: 5 }),
      ...
      expect(result).toStrictEqual(
        new SearchResult({ items: [items[0], items[2]], total: 3, current_page: 1, per_page: 2 }),
      );

      result = await repository.search(new SearchParams({ page: 2, per_page: 2, filter: 'TEST' }));
      expect(result).toStrictEqual(new SearchResult({ items: [items[3]], total: 3, current_page: 2, per_page: 2 }));
    });
      ...
      const arrange = [
        {
          ...
          search_result: new SearchResult({ items: [items[1], items[0]], total: 5, current_page: 1, per_page: 2 }),
        },
        {
          search_params: new SearchParams({ page: 2, per_page: 2, sort: 'name' }),
          search_result: new SearchResult({ items: [items[4], items[2]], total: 5, current_page: 2, per_page: 2 }),
        },
        {
          search_params: new SearchParams({ page: 1, per_page: 2, sort: 'name', sort_dir: 'desc' }),
          search_result: new SearchResult({ items: [items[3], items[2]], total: 5, current_page: 1, per_page: 2 }),
        },
        {
          search_params: new SearchParams({ page: 2, per_page: 2, sort: 'name', sort_dir: 'desc' }),
          search_result: new SearchResult({ items: [items[4], items[0]], total: 5, current_page: 2, per_page: 2 }),
        },
      ];
    ...
    test('should search using filter, sort and pagination', async () => {
      const items = [
        new StubEntity({ name: 'test', price: 5 }),
        new StubEntity({ name: 'a', price: 5 }),
        new StubEntity({ name: 'TEST', price: 5 }),
